...
🚧 **GATE — Mandatory read first**: `read_file templates/reference/design_spec_reference.md` before any analysis or writing. The design_spec.md output MUST follow that template's 11-section structure exactly. After writing, self-check each section is present: I Project Info → II Canvas → III Visual Theme → IV Typography → V Layout → VI Icon → VII Visualization → VIII Image → IX Outline → X Speaker Notes → XI Tech Constraints.

### Scenario Profile Gate

Before drafting the eight confirmations, load the academic scenario profiles:
...
Pick the most likely profile from the source, audience, and occasion, and
present it as a recommendation the user may change. Available profile ids are
`single_paper_report`, `multi_paper_review`, `thesis_defense`,
`proposal_or_fund`, `lab_progress`, `workshop_training`, and
`conference_talk`.

Treat these as seed profiles rather than the full academic universe. If the
deck is another scholarly occasion, choose the nearest rule base and record a
`scenario_variant`; do not force defense or literature-report semantics onto a
different academic use case.

For `single_paper_report`, also read
`references/literature-report-flow-selection.md` before the confirmations.
Detect whether the user supplied an outline, script, speaking notes, or
slide-by-slide plan. If yes, preserve that structure and do not substitute an
automatic long literature-report spine. If no, choose or present
`paper_ppt_concise` for short 6-10 slide figure-first reports and
`literature_report_deep_dive` for 20+ page deep literature reports requiring
`figure_source_manifest`, `deck_order_map`, and `page_briefs`.

Use the profile to separate structural obligation from preference:
- `hard_rules`: always enforced across academic decks. These protect source
  faithfulness, traceability, citation retention, text fit, template geometry,
  and PPTX deliverability.
- `required_rules`: enforced for this scenario unless the user changes the
  scenario.
- `recommended_rules`: good defaults such as action titles, ghost-deck checks,
  figure-first pages, or conclusion placement. Use them when they help, but do
  not force them through a conflicting template.
- `relaxable_rules`: soft defaults that may be relaxed for workshops, progress
  meetings, time-boxed training, or strong template constraints.

Use scenario first, template second. Templates control visual containers;
scenario profiles control content organization. A template route is not a
scenario.
A loaded template may control palette, title treatment, divider style, icon
usage, background treatment, page chrome, and layout density when the selected
profile lists those items in `template_may_override`. It must not override
`template_must_not_override` items such as source traceability, citation
retention, text fit, and deliverability.
...
academic spine. Keep slide copy audience-facing; do not leak template, slot,
placeholder, executor, renderer, generation, pipeline, or implementation
language into visible slide text unless the talk is explicitly about software
development.

### Deck Plan Contract Gate

After the eight confirmations are approved and before writing `spec_lock.md`,
create `<project_path>/deck_plan.json` and validate it:

```bash
python scripts/deck_plan_contract.py <project_path>/deck_plan.json --json
```

The deck plan is the page-level story contract. It must include:
- `schema_version: easyslides.deck_plan.v1`
- `scenario_profile`: the selected scenario profile id
- `source_map`: declared paper, figure, table, data, reference, or user-source ids
- `slides`: one object per page, each with `page`, `role`, `action_title`,
  `claim`, `evidence_sources`, `layout_id`, `rhythm`, and `speaker_note`

Use `action_title` for the slide's conclusion sentence, not a topic label.
Use `claim` for the precise statement the page must support. Use
`evidence_sources` to point back to `source_map` entries with a locator such as
page number, section, figure id, table id, or user-provided asset path. Use
...
Treat errors as blocking: a generic topic-label `action_title`, a result page
without figure/table/data/chart evidence, or a broken deck-plan contract means
...
**Style-pack descriptors**: when the user explicitly asks for Guizang / 归藏, Swiss internationalism, electronic magazine, or electronic ink, treat it as a reusable style-pack request, not just a vague descriptor. Lock the visual descriptor in `design_spec.md`, then write a `## style_pack` section in `spec_lock.md`:

- Guizang Style A / electronic magazine / electronic ink → `package: guizang_ppt`, `variant: style_a_editorial_ink`, default `theme: monocle_classic` unless the user names one of the five Style A themes.
- Guizang Style B / Swiss / grid-driven / IKB → `package: guizang_ppt`, `variant: style_b_swiss`, default `theme: ikb` unless the user names `lemon`, `lemon_green`, or `safety_orange`.

After locking a Guizang style pack, select `page_layouts` from `templates/style_packs/guizang_ppt/layouts.json`; for Style B prefer exact `S01`-`S22` IDs when the content shape matches an upstream Swiss layout.
...
>
> 1. **Pick exactly one stylistic library** — read the source material, then choose the library whose visual character best serves the deck. Prefer `lucide` for new generic icons and emoji replacement unless the template explicitly locks a different family:
>    - **`chunk-filled`** — fill, straight-line geometry (M/L/H/V/Z only); sharp right angles; heavy, solid, architectural
>    - **`lucide`** — stroke, clean neutral interface icons; recommended default, theme-colored via `fill`
...
>    - ⚠️ **One presentation = one stylistic library** for generic icons (home, chart, users, etc.). Mixing `lucide` / `chunk-filled` / `tabler-filled` / `tabler-outline` / `phosphor-duotone` is FORBIDDEN. If the chosen library lacks an exact icon, find the closest alternative **within that same library** or add the needed Lucide glyph before choosing `lucide`.
...
> 2. **Stroke weight lock (stroke-style libraries only)** — for stroke-based libraries (currently `lucide` and `tabler-outline`), pick one deck-wide value from `{1.5, 2, 2.25, 3}` (default `2`). For heavier presence, switch library instead of going above `3`.
>
...
>
> **Read-audit (mandatory, section VII format)** — single combined table; `summary-quote` column is the anti-fabrication audit, `path` + `usage` serve Executor lookup. Format defined in [`templates/reference/design_spec_reference.md`](../templates/reference/design_spec_reference.md) §VII:
> ```
...
4. **Generate execution lock**: read `templates/reference/spec_lock_reference.md` and produce `projects/<project_name>.../spec_lock.md` — a distilled, machine-readable short form of the color / typography / icon / image / **page_rhythm** / **page_layouts** / **page_charts** decisions above. This file is what the Executor re-reads before every page (see [executor-base.md](executor-base.md) §2.1). The values in `spec_lock.md` MUST exactly match the decisions recorded in `design_spec.md`; if they ever diverge, `spec_lock.md` wins and `design_spec.md` should be treated as historical narrative.
   ...
   - **style_pack (write when a reusable style package is selected)**: For Guizang, include `package`, `variant`, `theme`, `layout_source`, and `validator` rows. Then choose `page_layouts` from the style pack registry instead of inventing names. Validate with `python scripts/style_pack_contract.py <project_path>/spec_lock.md --json` before handing off to Executor.
   - **page_layouts (write only when a template is in use)**: For each page that inherits a template SVG, add `P<NN>: <svg_basename>` (e.g., `P04: 03a_content_image_text`). Pages designed freely get **no entry** — Executor reads the absence as "free design, no inheritance". If zero pages use a template, omit the section entirely.
     - With `style_pack: guizang_ppt`, values may be Guizang registry IDs (`S01`-`S22`) or package-relative basenames (`swiss/S08_duo_compare`). Do not write an `Sxx` ID unless it exists in `templates/style_packs/guizang_ppt/layouts.json`.
