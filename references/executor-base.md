...
|---|---|
| Chosen template's `design_spec.md` (read frontmatter to detect `replication_mode`) | `templates/layouts/<chosen_template>/design_spec.md` |
| Every distinct `<basename>` in `spec_lock.md page_layouts` | `templates/layouts/<chosen_template>/<basename>.svg` |
| If `spec_lock.md style_pack` is present, every distinct style-pack layout in `page_layouts` | Resolve with `python scripts/style_pack_contract.py <project_path>/spec_lock.md --json`, then read each reported `resolved_layouts.*.path` |
...
1. **Style-pack contract** (`spec_lock.md` has `## style_pack`) and `page_layouts` has an entry for this page → resolve the value through `scripts/style_pack_contract.py` and inherit the reported SVG path. Example: Guizang `P04: S08` resolves to `templates/style_packs/guizang_ppt/swiss/S08_duo_compare.svg`.
2. **Mirror-mode template** (template's `design_spec.md` frontmatter has `replication_mode: mirror`) → see §1.1 below. The page is consumed as a **visual reference**, not as a placeholder shell.
3. `spec_lock.md page_layouts` has `P<NN>: <basename>` for this page → inherit the structure of `templates/layouts/<chosen_template>/<basename>.svg` (already in context from §1.0).
4. `page_layouts` exists but **no entry** for this page → **free design**, no template inheritance.
5. `page_layouts` section absent (legacy deck) **and** `templates/` directory exists → fall back to the page-type table below, matching by SVG filename keyword (cover/chapter/content/ending/toc). Read the matched file at first use if §1.0 batch did not cover it.
6. No template at all → free design.
...
```
📝 **Template mapping**: `templates/layouts/<chosen_template>/03a_content_image_text.svg` (or "None (free design)")
...
- If `style_pack` is present, use the style-pack contract first. Guizang accepts canonical IDs (`S01`-`S22`) and package-relative SVG names (`swiss/S08_duo_compare`); missing or unknown values should be surfaced as `warning: page_layouts P<NN> references unknown style-pack layout <value>`.
...
<!-- lucide (clean line icons; recommended default for generic icons / emoji replacement) -->
<use data-icon="lucide/calendar-days" x="100" y="200" width="48" height="48" fill="#005587" stroke-width="2"/>
...
<!-- stroke-style libraries with thin / bold stroke -->
...
>
> **stroke-width** (stroke-style libraries only, currently `lucide` and `tabler-outline`): allowed values `{1.5, 2, 2.25, 3}`. If `spec_lock.md icons.stroke_width` is declared, all placeholders MUST use that value deck-wide. Default `2` if absent (legacy). Ignored on non-stroke libraries.
>
