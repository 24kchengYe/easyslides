...
class SVGQualityChecker:
    ...
    def _check_image_references(self, content: str, svg_path: Path, result: Dict):
        ...
        for tag_match in img_tag_pattern.finditer(content):
            ...
            # Library templates often declare image slots as placeholders such
            # as {{LOGO_IMAGE}}. Those are filled by the deck generator and are
            # not real filesystem paths at template validation time.
    ...
    def _extract_spec_roster(spec_text: str) -> List[str]:
        ...
        # mentions elsewhere.
        explicit_svg_roster = bool(scope and re.search(r"[`\(][0-9A-Za-z_]+\.svg[`\)]", scope))
        if explicit_svg_roster:
            text = scope
        else:
            text = spec_text
        ...
        for match in svg_ref_re.finditer(text):
            ...
            if stem in seen:
                continue
            if not explicit_svg_roster and not re.match(r"^\d", stem):
                continue
            seen.add(stem)
            stems.append(stem)
...
def print_usage() -> None:
    ...
    print("  python3 scripts/svg_quality_checker.py templates/layouts/defense_leftnav --template-mode")
    print("\nOptions:")
