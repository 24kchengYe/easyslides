...
class ProjectManager:
    ...
    def _import_pdf(self, pdf_path: Path, markdown_path: Path) -> None:
        ...
        if mineru_mod.exists():
            try:
                from source_to_md.mineru_preprocess import extract_pdf as mineru_extract
            except ImportError:
                try:
                    import importlib.util
                    spec = importlib.util.spec_from_file_location(
                        "mineru_preprocess", str(mineru_mod)
                    )
                    mod = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(mod)
                    mineru_extract = mod.extract_pdf
                except Exception:
                    mineru_extract = None

            if mineru_extract is not None:
                try:
