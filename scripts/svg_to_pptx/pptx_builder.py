...
from .pptx_notes import (
    ...
    create_notes_master_xml, create_notes_master_rels_xml,
)
...
def _ensure_notes_master(extract_dir: Path, content_types: str) -> str:
    """Create notes master parts and return updated [Content_Types].xml."""
    notes_master_dir = extract_dir / 'ppt' / 'notesMasters'
    notes_master_dir.mkdir(exist_ok=True)
    ...
    theme_name = 'theme1.xml'
    if not (theme_dir / theme_name).exists():
        themes = sorted(theme_dir.glob('theme*.xml'))
        if themes:
    ...
    notes_master_rels_path.write_text(
        create_notes_master_rels_xml(theme_name),
        encoding='utf-8',
    )
    ...
    for match in re.finditer(
        r'<Relationship\s+Id="(rId\d+)"[^>]+Type="http://schemas\.openxmlformats\.org/officeDocument/2006/relationships/notesMaster"[^>]+Target="notesMasters/notesMaster1\.xml"',
        pres_rels,
    ):
        notes_master_rid = match.group(1)
        break
    ...
    if '<p:notesMasterIdLst' not in presentation_xml:
        notes_master_xml = (
    ...
    if notes_master_override not in content_types:
        content_types = content_types.replace('</Types>', notes_master_override + '\n</Types>')
