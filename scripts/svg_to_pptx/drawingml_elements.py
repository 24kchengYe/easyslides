...
def _is_truthy_attr(elem: ET.Element, name: str) -> bool:
    """Return whether a custom SVG flag is enabled."""
    return (elem.get(name) or "").strip().lower() in {"1", "true", "yes"}


def _is_line_start_tspan(tspan: ET.Element) -> bool:
    """Return whether a tspan encodes a new visual line."""
    if tspan.get("y") is not None or tspan.get("x") is not None:
        return True
    dy = tspan.get("dy")
    if dy is None:
        return False
    try:
        return float(str(dy).strip().split()[0].rstrip("px")) != 0
    except (ValueError, IndexError):
        return bool(str(dy).strip())


def _normalize_paragraph_runs(
    paragraphs: list[list[dict[str, Any]]],
    preserve_space: bool,
) -> list[list[dict[str, Any]]]:
    """Trim paragraph edges and drop empty runs."""
...
def _build_text_paragraphs(
    elem: ET.Element,
    parent_attrs: dict[str, Any],
) -> list[list[dict[str, Any]]]:
    """Build editable paragraphs from a text element's positional tspans.

    ``data-pptx-textbox=true`` opts out of the normal tspan flattening route:
    each visual line remains inside one DrawingML text box as a separate
    paragraph. This is for template body slots where one editable text frame is
    more important than one-shape-per-line fidelity.
    """
    preserve_space = _preserves_space(elem)
    paragraphs: list[list[dict[str, Any]]] = []
    ...
    if elem.text:
        t = _normalize_text(elem.text, preserve_space=preserve_space)
        if t:
            current.append({**parent_attrs, "text": t})
    ...
    for child in elem:
        ...
        if _is_line_start_tspan(child) and current:
            paragraphs.append(current)
            current = []

        current.extend(_collect_tspan_runs(child, parent_attrs, preserve_space))
        if child.tail:
            t = _normalize_text(child.tail, preserve_space=preserve_space)
            if t:
                current.append({**parent_attrs, "text": t})
    ...
    if not paragraphs:
        paragraphs = [_build_text_runs(elem, parent_attrs)]

    return _normalize_paragraph_runs(paragraphs, preserve_space)
...
def convert_text(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    keep_textbox = _is_truthy_attr(elem, 'data-pptx-textbox')
    paragraphs = (
        _build_text_paragraphs(elem, parent_attrs)
        if keep_textbox
        else [_build_text_runs(elem, parent_attrs)]
    )
    ...
    line_texts = [''.join(r['text'] for r in p) for p in paragraphs]
    text_width = max(
        estimate_text_width(line_text, font_size, font_weight)
        for line_text in line_texts
    ) * 1.15
    text_height = font_size * (1.35 * len(paragraphs) if keep_textbox else 1.5)
    ...
    paragraphs_xml = '\n'.join(
        '<a:p>\n'
        f'<a:pPr algn="{algn}"/>\n'
        + '\n'.join(_build_run_xml(r, fonts, ctx, text_effect_xml) for r in paragraph)
        + '\n</a:p>'
...
<p:txBody>
<a:bodyPr wrap="{wrap_mode}" lIns="0" tIns="0" rIns="0" bIns="0" anchor="{body_anchor}" anchorCtr="0">
{autofit_xml}
</a:bodyPr>
<a:lstStyle/>
{paragraphs_xml}
