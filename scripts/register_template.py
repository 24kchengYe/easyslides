...
def _patch_readme(
    ...
    *,
    ...
    text = README_PATH.read_text(encoding="utf-8-sig") if README_PATH.exists() else ""

    # Update header count
    new_total = len(items)
    text = re.sub(
        r"^# (?:Academic Layout Template Library|Page Layout Template Library) \(\d+(?: Active)? Templates\)",
        f"# Academic Layout Template Library ({new_total} Active Templates)",
        text,
        count=1,
        flags=re.MULTILINE,
    )
